...
def get_database_connection():
    """Create database connection - supports both SQLite (local) and CSV (cloud)"""
    import os

    # Try local Windows path first (for your PC)
    local_path = r'C:\Users\vmminhas\Desktop\Python\Machine Learning\master_consumption.db'
    if os.path.exists(local_path):
        return sqlite3.connect(local_path, check_same_thread=False), 'sqlite'

    # Try current working directory (for Streamlit Cloud)
    cwd_path = os.path.join(os.getcwd(), 'master_consumption.db')
    if os.path.exists(cwd_path):
        return sqlite3.connect(cwd_path, check_same_thread=False), 'sqlite'

    # Try /mount/src/consumption/ (Streamlit Cloud default mount path)
    cloud_path = '/mount/src/consumption/master_consumption.db'
    if os.path.exists(cloud_path):
        return sqlite3.connect(cloud_path, check_same_thread=False), 'sqlite'
